...
#
#              for k in range(r - s):       It won't include s (you ned s+1)
#                  count = 0                count should be outside the loop
#                  if math.cos(r) > 0:      should >=0
#                      count = 1            count+=1
#                  return count             should be outside of the for loop
#
#     f. The code in the "graphics accumulation" example below includes:
#            for _ in range(n):
#        What does the   _   (underscore) mean?
#       it's the local variable in the for loop that increments every iteration (usually k)
...
#
#           The first one is wrong because nothing changes every time it loops through.
#              x = starting_point.x
#              for k in range(n):
